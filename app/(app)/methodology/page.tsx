...
export default async function Methodology() {
  const [s, d] = await Promise.all([platformStatus(), loadCf()]);
  ...
  const status: StatusRow[] = [
    ...
    { icon: 'timer', label: 'Snapshot and refresh cadence', detail: 'Tokens, pools and flows are snapshotted hourly and kept at daily grain. These pages revalidate every five minutes.' },
    { icon: 'pulse', label: 'Trend window', detail: `${config.trend.days} days of daily TVL rows behind the overview trend; ${config.flowDays} days of flows behind the flow figures; token history from ${config.tokenHistoryFrom} where the API has it.`, badge: { text: `${d.history.length} days`, tone: 'neutral' } },
          ...
          <CardContent>
            {r ? (
              <div className="grid grid-cols-1 gap-4 @2xl/main:grid-cols-[1fr_1fr_2fr]">
                <div className="rounded-lg border bg-muted/40 p-4"><div className="text-xs text-muted-foreground">Our TVL, every pool</div><div className="mt-1 text-2xl font-medium tracking-tight tabular-nums">{usd(r.ours)}</div><div className="mt-1 text-xs text-muted-foreground">as of {d.asOf}</div></div>
