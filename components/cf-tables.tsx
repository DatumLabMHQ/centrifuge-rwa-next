...
const flowColumns = defineColumns<Flow>((col) => [
  ...
  col.accessor('net', { header: ({ column }) => <SortHeader column={column} label="Net flow" />, cell: ({ row }) => <span className={`tabular-nums ${row.original.net < 0 ? 'text-(--red)' : row.original.net > 0 ? 'text-(--green)' : 'text-muted-foreground'}`}>{row.original.net < 0 ? '-' : '+'}{usd(Math.abs(row.original.net))}</span> }),
